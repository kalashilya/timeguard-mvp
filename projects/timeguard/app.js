(() => {
  ...
  const keys = { profile: 'timeguard_profile_v1', tasks: 'timeguard_tasks_v1', stats: 'timeguard_stats_v1' };
  ...
  function toggleDone(id) { tasks = tasks.map(t => t.id === id ? { ...t, done: !t.done } : t); save(); renderAll(); }
  ...
  function openPaywall() { $('paywallModal')?.classList.remove('hidden'); toast('Достигнут Free-лимит'); }
